...
async function cellEvent(event) {
  ...
  if ((keyValue === "BACKSPACE" || keyValue === "DELETE") && focusColumn > 0) {
    if (focusColumn < maxColumn)
      changeClass(idBox, { style: "border-color", value: "#808080" });
    changeBox("column", -1);
  ...
  // Si on est en fin de ligne et que le input est la touche entrée
    ...
    if (data.result || debug) {
      ...
      checkArr.map((color, index) => {
        ...
        changeClass(
          box,
          { class: "box-flip", style: "animation-delay", value: delay + "ms" },
          delay + 1000
        );
        ...
        if (
          color === "green" ||
          (color === "yellow" &&
            selectedKeyboard.style.backgroundColor !== "rgb(83, 141, 78)") ||
          (color === "grey" &&
            selectedKeyboard.style.backgroundColor !== "rgb(83, 141, 78)" &&
            selectedKeyboard.style.backgroundColor !== "rgb(181, 159, 59)")
        )
          changeColor = true;
  ...
  try {
    ...
    for (let i = 0; i < maxRow; i++) {
      ...
      for (let j = 0; j < maxColumn; j++)
        gameBoard +=
          '<div id="box-' + i + "-" + j + '" class="letter-box"></div>';
      gameBoard += "</div>";
    }
...
function sizeGrid(event) {
  ...
  if (newLength >= rangeLength[0] && newLength <= rangeLength[1]) {
    ...
    if (newLength >= rangeRow[0] && newLength <= rangeRow[1])
      maxRow = newLength;
    ...
    if (document.getElementById("up").disabled !== abordUp)
      document.getElementById("up").disabled = abordUp;
    if (newLength === rangeLength[0]) abordDown = true;
    if (document.getElementById("down").disabled !== abordDown)
      document.getElementById("down").disabled = abordDown;
...
document
  .getElementById("down")
  .addEventListener("click", () => sizeGrid(document.getElementById("down")));
createGrid();
